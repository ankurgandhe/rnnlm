    ...
    def __init__(self, input, n_in, n_hidden, n_out, activation=T.tanh,
 	...
 	'''
        ...
        [self.h, self.y_pred], _ = theano.scan(step,
					       ...
					       outputs_info=[dict(initial=self.h0, taps=[-1]),None])
        ...
        self.y_pred = self.y_pred.reshape((7,3)) #n_out))
...
class MetaRNN():
    ...
    def ready(self):
        ...
        self.y = T.ivector() #name='y', dtype='int32')
...
def test_softmax(n_epochs=250):
    ...
    # simple lag test
    seq = np.random.randn(n_seq, n_steps, n_in)
    targets = np.zeros((n_seq, n_steps), dtype=np.int32)
    ...
    #targets[:, 2:, 0] = np.cast[np.int](seq[:, 1:-1, 3] > seq[:, :-2, 0])
    print seq.shape,targets.shape,targets.dtype 
    model = MetaRNN(n_in=n_in, n_hidden=n_hidden, n_out=n_out,
                    learning_rate=0.001, learning_rate_decay=0.999,
